function getInterfaceAlias(a, b, c, d) {
    var e = $.xmo.getValuesTree($.xpaths.adminAdvanced.connectedLTE);
    return a && "1" !== a || "UP" !== e ? "DSLITE_ENTRY" === b ? c && "ipv4" === c.toLowerCase() ? "DSLITE_ENTRY" : "IP_DATA" : "IP_BR_LAN" === d ? d : void 0 : "IP_LTE"
}
$.xmo.StringTree,
$.xmo.StringOrTree,
$.xmo.NStringTree,
...
$.xmo.err = {
    ...
    XMO_CHANNEL_BANDWIDTH_INCOMPATIBILITY_ERR: 16777321
},
$.xmo.init = function() {
    $.gui.init("en", "gtw")
}
...
$.xmo.isRequestError = function(a) {
    return a.code !== $.xmo.err.XMO_NO_ERR && a.code !== $.xmo.err.XMO_REQUEST_NO_ERR
}
,
$.xmo.createSafeOptions = function(a) {
    a = $.extend(defautOptions, a);
    var b = a.success
      , c = a.error
      , d = function(c) {
        var d = c.error;
        return d && $.xmo.isRequestError(d) ? void a.error(c) : void b(c)
    }
      , e = function() {
        c(arguments)
    };
...
$.xmo.logout = function(a) {
    ...
    $.xmo.session = 0) : $.xmo.logger.log("Not logged in."),
    $.cookie("session", null, {
        path: "/"
    }),
    $.xmo.client = null,
...
$.xmo.restoreConfiguration = function(a, b) {
    var c = b ? b.success : function() {}
      , d = b ? b.error : function() {}
    ;
    ...
    return $.config.modules.restoreConfigurationAllBackup === !0 ? e.uploadSpecificFile($.xpaths.mySagemcomBox.maintenance.saveRestore.restore, $(a), 1, !0, function(a) {}, function(a) {}) : e.uploadFile($.xpaths.mySagemcomBox.maintenance.saveRestore.restore, $(a), function(a) {}, function(a) {
        validRestoreFile = !1
    }),
    e.send(c, d),
    validRestoreFile
...
,
$.xmo.ringTestRPC = function(a) {
    if (!$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("ringTestRPC error: Not logged in");
    var b = $.xmo.client.newRequest();
    b.remoteCall($.xpaths.main.ringTest.rpc, "testPhones"),
    b.send(a.success, a.error)
}
,
$.xmo.docsisFirmwareUpgrade = function(a, b) {
    if (!$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("firmwareUpgrade error: Not logged in");
    var c = $.xmo.client.newRequest();
    c.docsisFirmwareUpgrade($.xpaths.rpc.firmwareUpgrade, $(a), function() {}, function() {
        throw new Error("An error has occurred")
    }, {
        fileType: b
    }),
...
,
$.xmo.getInterfaceXPathByWanProvisioningMode = function(a, b, c, d, e) {
    var f = "dslite";
    if (e)
        g = e;
    else {
        var g = "" !== a && a.toLowerCase() === f && "" !== b && "ipv4" === b.toLowerCase() ? "DSLITE_ENTRY" : "IP_DATA"
          , h = getInterfaceAlias(d, g, b, c);
        d && "1" !== d ? "2" === d ? g = h ? h : g : "3" === d && (g = "IP_LTE") : g = h ? h : g
    }
    return $.xmo.getPingInterfaceXpath(g)
}
,
...
,
$.xmo.ping = function(a, b, c, d, e, f, g, h, i) {
    var j = "";
    if (j = i ? $.xmo.getInterfaceXPathByWanProvisioningMode(e, f, g, h, i) : $.util.hasFeature("pingInterface") ? $.xmo.getPingInterfaceXpath(g) : $.xmo.getInterfaceXPathByWanProvisioningMode(e, f, g, h, ""),
    c || "" !== c || (c = "AUTO"),
    !$.xmo.loggedin())
    ...
    m.remoteCall($.xpaths.trafficStats.rpc, "ping", {
        Host: a,
        Count: b,
        Protocol: c,
        Interface: j
    ...
    var n = function(a) {
        ...
        l = a.error,
        k = a.actions[0].callbacks[0].parameters,
        l.code !== $.xmo.err.XMO_REQUEST_NO_ERR ? k.Status = "Inactive" : k.Status = "Active",
        d(k)
    };
    return m.send(n, n, {
        synchronous: !1
...
,
$.xmo.traceroute = function(a, b, c, d, e, f, g) {
    var h = $.xmo.getInterfaceXPathByWanProvisioningMode(d, e, "", f, g);
    if (e || (e = "AUTO"),
    !$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("traceroute error: Not logged in");
    var i = $.xmo.client.newRequest();
    i.remoteCall($.xpaths.trafficStats.rpc, "traceRoute", {
        Host: a,
        Protocol: e,
        Count: b,
        Interface: h
...
$.xmo.addChildrenMultiple = function(a, b, c) {
    ...
    return c ? d : (b = $.xmo.createSafeOptions(b),
    void d.send(b.success, b.error, {
        synchronous: b.sync
    }))
}
,
...
$.xmo.getValuesFlatSplit = function(a, b, c) {
    ...
    g = 90);
    for (var i = {}, j = Object.keys(a), k = 0; f >= g; ) {
        for (i = {},
        d = h; g > d; d++)
            i[j[d]] = a[j[d]];
        ...
        a = $.extend(a, i),
        g >= f ? f = 0 : (h = g,
        f % 90 === 0 ? g += 90 : f - g >= 90 ? g = f - (f - 90 * k) : g += f - g),
        k++
    }
    return a
}
...
$.xmo.cancelNotification = function(a) {
    if (a && $.xmo.idNotif && 0 !== $.xmo.idNotif.length) {
        try {
            var b = $.xmo.client.newRequest();
...
,
$.xmo.iperfRpc = function(a, b) {
    if (!$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("DHCP server list error: Not logged in");
    var c = $.xmo.client.newRequest();
    c.remoteCall($.xpaths.startSpeed.rpc, "getIperf3LogDownloadURI"),
    c.send(function(b) {
        a(b.actions[0].callbacks[0].parameters.uri)
    }, b)
}
,
...
$.xmo.getModel = function() {
    ...
    var a = $.xmo.loggedin();
    if (!$.xmo.loggedin() && !$.util.getModuleConfig("myBox.maintenance.noAutoLoginGuest", !1))
        try {
...
,
$.xmo.getBusinessEnable = function() {
    $.xmo.init(),
    $.xmo.restoreFromCookie();
    var a = $.xmo.loggedin();
    if (!$.xmo.loggedin() && !$.util.getModuleConfig("myBox.maintenance.noAutoLoginGuest", !1))
    ...
    var c = $.xmo.getValuesTree($.xpaths.businessEnable);
    if (!a)
        try {
            $.xmo.logout(!0)
        } catch (d) {}
...
,
$.xmo.getProfileDetection = function(a) {
    $.xmo.init(),
    $.xmo.restoreFromCookie();
    var b = $.xmo.loggedin();
    if (!$.xmo.loggedin() && !$.util.getModuleConfig("myBox.maintenance.noAutoLoginGuest", !1))
...
$.xmo.getProductClass = function() {
    ...
    var c = $.xmo.getValuesTree($.xpaths.mySagemcomBox.deviceInfo.productClass);
    if (!a)
        try {
            $.xmo.logout(!0)
        } catch (d) {}
...
$.xmo.getCountry = function() {
    ...
    var c = $.xmo.getValuesTree($.xpaths.management.country);
    if (!a)
        try {
            $.xmo.logout(!0)
        } catch (d) {}
...
$.xmo.getHwVersion = function() {
    ...
    var c = $.xmo.getValuesTree($.xpaths.mySagemcomBox.deviceInfo.hardwareVersion);
    if (!a)
        try {
            $.xmo.logout(!0)
        } catch (d) {}
...
,
$.xmo.uploadGlobalDataUsage = function(a) {
    if (!$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("Upload global data usage error: Not logged in");
    var b = function(a) {
        return a && a.actions && a.actions.length && a.actions[0].callbacks && a.actions[0].callbacks && a.actions[0].callbacks[0] && a.actions[0].callbacks[0].parameters && a.actions[0].callbacks[0].parameters.data ? JSON.parse(a.actions[0].callbacks[0].parameters.data.replace(/\n/g, "")) || "" : ""
    };
    a || (a = {
        days: 30,
        success: function() {},
    ...
    var c = "DD-MM-YYYY"
      , d = moment().format(c)
      , e = moment().subtract(a.days || 30, "days").format(c)
      , f = $.xmo.client.newRequest();
    f.remoteCall($.xpaths.rpc.device, "uploadGlobalDataUsage", {
        startDate: e,
        endDate: d
    }),
    f.send(function(c) {
...
,
$.xmo.renewSession = function() {
    if (!$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("Errrror: Not logged in");
    $.xmo.getValuesTree($.xpaths.mySagemcomBox.deviceInfo.upTime)
}
,
$.xmo.uploadDetailledDataUsage = function(a, b) {
    if (!$.xmo.loggedin())
        throw new $.xmo.NotLoggedInError("Upload detailed data usage error: Not logged in");
    var c = function(a) {
        return a && a.actions && a.actions.length && a.actions[0].callbacks && a.actions[0].callbacks && a.actions[0].callbacks[0] && a.actions[0].callbacks[0].parameters && a.actions[0].callbacks[0].parameters.data ? a.actions[0].callbacks[0].parameters.data || "" : ""
    };
    b || (b = {
        hours: 24,
        success: function() {},
    ...
    var d = $.xmo.client.newRequest();
    d.remoteCall($.xpaths.rpc.device, "uploadDetailledDataUsage", {
        filterStr: a,
        maxLastCollectingHours: b.hours || 24
    }),
    d.send(function(a) {
        b.success(c(a))
    }, b.error)
